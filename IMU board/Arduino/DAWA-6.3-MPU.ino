...
/**************************************************************
  DAWA Extension - var+sercom
**************************************************************/
#define DEBUG_PORT SERIAL_PORT_USBVIRTUAL // Debug to USB serial console
#define I2C_SLAVE_ADDRESS 18 // I2C slave address for DAWA
...
void setup()
{
  ...
  DEBUG_PORT.println("MPU9250 9-axis motion sensor...");
  byte c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
  DEBUG_PORT.print("MPU9250 "); DEBUG_PORT.print("I AM "); DEBUG_PORT.print(c, HEX); DEBUG_PORT.print(" I should be "); DEBUG_PORT.println(0x71, HEX);
  ...
  {
    DEBUG_PORT.println("MPU9250 is online...");

    MPU9250SelfTest(SelfTest); // Start by performing self test and reporting values
    DEBUG_PORT.print("x-axis self test: acceleration trim within : "); DEBUG_PORT.print(SelfTest[0], 1); DEBUG_PORT.println("% of factory value");
    DEBUG_PORT.print("y-axis self test: acceleration trim within : "); DEBUG_PORT.print(SelfTest[1], 1); DEBUG_PORT.println("% of factory value");
    DEBUG_PORT.print("z-axis self test: acceleration trim within : "); DEBUG_PORT.print(SelfTest[2], 1); DEBUG_PORT.println("% of factory value");
    DEBUG_PORT.print("x-axis self test: gyration trim within : "); DEBUG_PORT.print(SelfTest[3], 1); DEBUG_PORT.println("% of factory value");
    DEBUG_PORT.print("y-axis self test: gyration trim within : "); DEBUG_PORT.print(SelfTest[4], 1); DEBUG_PORT.println("% of factory value");
    DEBUG_PORT.print("z-axis self test: gyration trim within : "); DEBUG_PORT.print(SelfTest[5], 1); DEBUG_PORT.println("% of factory value");
    delay(1000);
    ...
    DEBUG_PORT.println(" Calibrate gyro and accel");
    accelgyrocalMPU9250(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
    DEBUG_PORT.println("accel biases (mg)"); DEBUG_PORT.println(1000.*accelBias[0]); DEBUG_PORT.println(1000.*accelBias[1]); DEBUG_PORT.println(1000.*accelBias[2]);
    DEBUG_PORT.println("gyro biases (dps)"); DEBUG_PORT.println(gyroBias[0]); DEBUG_PORT.println(gyroBias[1]); DEBUG_PORT.println(gyroBias[2]);
    ...
    initMPU9250();
    DEBUG_PORT.println("MPU9250 initialized for active data mode...."); // Initialize device for active mode read of acclerometer, gyroscope, and temperature

    // Read the WHO_AM_I register of the magnetometer, this is a good test of communication
    byte d = readByte(AK8963_ADDRESS, AK8963_WHO_AM_I);  // Read WHO_AM_I register for AK8963
    DEBUG_PORT.print("AK8963 "); DEBUG_PORT.print("I AM "); DEBUG_PORT.print(d, HEX); DEBUG_PORT.print(" I should be "); DEBUG_PORT.println(0x48, HEX);
    /*display.clearDisplay();
    ...
    initAK8963(magCalibration); DEBUG_PORT.println("AK8963 initialized for active data mode...."); // Initialize device for active mode read of magnetometer

    magcalMPU9250(magBias, magScale);
    DEBUG_PORT.println("AK8963 mag biases (mG)"); DEBUG_PORT.println(magBias[0]); DEBUG_PORT.println(magBias[1]); DEBUG_PORT.println(magBias[2]);
    DEBUG_PORT.println("AK8963 mag scale (mG)"); DEBUG_PORT.println(magScale[0]); DEBUG_PORT.println(magScale[1]); DEBUG_PORT.println(magScale[2]);
    delay(2000); // add delay to see results before serial spew of data

    if (SerialDebug) {
      //  DEBUG_PORT.println("Calibration values: ");
      DEBUG_PORT.print("X-Axis sensitivity adjustment value "); DEBUG_PORT.println(magCalibration[0], 2);
      DEBUG_PORT.print("Y-Axis sensitivity adjustment value "); DEBUG_PORT.println(magCalibration[1], 2);
      DEBUG_PORT.print("Z-Axis sensitivity adjustment value "); DEBUG_PORT.println(magCalibration[2], 2);
    }
    ...
    MS5637Reset();
    delay(100);
    DEBUG_PORT.println("MS5637 pressure sensor reset...");
    // Read PROM data from MS5637 pressure sensor
    MS5637PromRead(Pcal);
    DEBUG_PORT.println("PROM dta read:");
    DEBUG_PORT.print("C0 = "); DEBUG_PORT.println(Pcal[0]);
    unsigned char refCRC = Pcal[0] >> 12;
    DEBUG_PORT.print("C1 = "); DEBUG_PORT.println(Pcal[1]);
    DEBUG_PORT.print("C2 = "); DEBUG_PORT.println(Pcal[2]);
    DEBUG_PORT.print("C3 = "); DEBUG_PORT.println(Pcal[3]);
    DEBUG_PORT.print("C4 = "); DEBUG_PORT.println(Pcal[4]);
    DEBUG_PORT.print("C5 = "); DEBUG_PORT.println(Pcal[5]);
    DEBUG_PORT.print("C6 = "); DEBUG_PORT.println(Pcal[6]);

    nCRC = MS5637checkCRC(Pcal);  //calculate checksum to ensure integrity of MS5637 calibration data
    DEBUG_PORT.print("Checksum = "); DEBUG_PORT.print(nCRC); DEBUG_PORT.print(" , should be "); DEBUG_PORT.println(refCRC);
  ...
  {
    DEBUG_PORT.print("Could not connect to MPU9250: 0x");
    DEBUG_PORT.println(c, HEX);
    while (1) ; // Loop forever if communication doesn't happen
  }

  /**************************************************************
    DAWA Extension - I2C Sercom init
  **************************************************************/
  Wire1.begin(I2C_SLAVE_ADDRESS);
  Wire1.onRequest(requestEvent); // Handle incoming data on I2C
}

/**************************************************************
  DAWA Extension - Function called when I2C master request
**************************************************************/
void requestEvent() {
  Wire1.write((byte*)&pitch, 4);
  Wire1.write((byte*)&roll, 4);
  Wire1.write((byte*)&ax, 4);
  Wire1.write((byte*)&ay, 4);
}
...
void loop()
{
  ...
  if (newData == true) { // On interrupt, read data
    //if(true) {  // On interrupt, read data
  ...
  if (delt_t > 100) { // update LCD once per half-second independent of read rate
    ...
    if (SerialDebug) {
      /*DEBUG_PORT.print("ax = "); DEBUG_PORT.print((int)1000*ax);
        DEBUG_PORT.print(" ay = "); DEBUG_PORT.print((int)1000*ay);
        DEBUG_PORT.print(" az = "); DEBUG_PORT.print((int)1000*az); DEBUG_PORT.println(" mg");
        DEBUG_PORT.print("gx = "); DEBUG_PORT.print( gx, 2);
        DEBUG_PORT.print(" gy = "); DEBUG_PORT.print( gy, 2);
        DEBUG_PORT.print(" gz = "); DEBUG_PORT.print( gz, 2); DEBUG_PORT.println(" deg/s");
        DEBUG_PORT.print("mx = "); DEBUG_PORT.print( (int)mx );
        DEBUG_PORT.print(" my = "); DEBUG_PORT.print( (int)my );
        DEBUG_PORT.print(" mz = "); DEBUG_PORT.print( (int)mz ); DEBUG_PORT.println(" mG");

        DEBUG_PORT.print("q0 = "); DEBUG_PORT.print(q[0]);
        DEBUG_PORT.print(" qx = "); DEBUG_PORT.print(q[1]);
        DEBUG_PORT.print(" qy = "); DEBUG_PORT.print(q[2]);
        DEBUG_PORT.print(" qz = "); DEBUG_PORT.println(q[3]); */
    }
    tempCount = readTempData();  // Read the gyro adc values
    temperature = ((float) tempCount) / 333.87 + 21.0; // Gyro chip temperature in degrees Centigrade
    // Print temperature in degrees Centigrade
    //DEBUG_PORT.print("Gyro temperature is ");  DEBUG_PORT.print(temperature, 1);  DEBUG_PORT.println(" degrees C"); // Print T values to tenths of s degree C
    ...
    if (SerialDebug) {
      /*DEBUG_PORT.print("Digital temperature value = "); DEBUG_PORT.print( (float)Temperature, 2); DEBUG_PORT.println(" C"); // temperature in degrees Celsius
        DEBUG_PORT.print("Digital temperature value = "); DEBUG_PORT.print(9.*(float) Temperature/5. + 32., 2); DEBUG_PORT.println(" F"); // temperature in degrees Fahrenheit
        DEBUG_PORT.print("Digital pressure value = "); DEBUG_PORT.print((float) Pressure, 2);  DEBUG_PORT.println(" mbar");// pressure in millibar
        DEBUG_PORT.print("Altitude = "); DEBUG_PORT.print(altitude, 2); DEBUG_PORT.println(" feet");*/
    }
    ...
    lin_az = az - a33;
    if (SerialDebug) {
      DEBUG_PORT.print("Yaw, Pitch, Roll: ");
      DEBUG_PORT.print(yaw, 2);
      DEBUG_PORT.print(", ");
      DEBUG_PORT.print(pitch, 2);
      DEBUG_PORT.print(", ");
      DEBUG_PORT.println(roll, 2);

      /*DEBUG_PORT.print("Grav_x, Grav_y, Grav_z: ");
        DEBUG_PORT.print(-a31 * 1000, 2);
        DEBUG_PORT.print(", ");
        DEBUG_PORT.print(-a32 * 1000, 2);
        DEBUG_PORT.print(", ");
        DEBUG_PORT.print(a33 * 1000, 2);  DEBUG_PORT.println(" mg");
        DEBUG_PORT.print("Lin_ax, Lin_ay, Lin_az: ");
        DEBUG_PORT.print(lin_ax * 1000, 2);
        DEBUG_PORT.print(", ");
        DEBUG_PORT.print(lin_ay * 1000, 2);
        DEBUG_PORT.print(", ");
        DEBUG_PORT.print(lin_az * 1000, 2);  DEBUG_PORT.println(" mg");*/

      DEBUG_PORT.print("rate = "); DEBUG_PORT.print((float)sumCount / sum, 2); DEBUG_PORT.println(" Hz");
    }
...
void magcalMPU9250(float * dest1, float * dest2)
{
  ...
  DEBUG_PORT.println("Mag Calibration: Wave device in a figure eight until done!");
  delay(4000);
  ...
  //    DEBUG_PORT.println("mag x min/max:"); DEBUG_PORT.println(mag_max[0]); DEBUG_PORT.println(mag_min[0]);
  //    DEBUG_PORT.println("mag y min/max:"); DEBUG_PORT.println(mag_max[1]); DEBUG_PORT.println(mag_min[1]);
  //    DEBUG_PORT.println("mag z min/max:"); DEBUG_PORT.println(mag_max[2]); DEBUG_PORT.println(mag_min[2]);
  ...
  DEBUG_PORT.println("Mag Calibration done!");
}
